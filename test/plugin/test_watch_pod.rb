...
class WatchPodTestTest < WatchTest
     ...
     test 'pod watch updates cache when MODIFIED is received' do
      ...
      @client.stub :watch_pods, [@modified] do
       start_pod_watch
       assert_equal(true, @cache.key?('original_uid'))
    ...
    test 'pod watch updates cache when ADDED is received' do
      @client.stub :watch_pods, [@added] do
       start_pod_watch
       assert_equal(true, @cache.key?('added_uid'))
       assert_equal(1, @stats[:pod_cache_watch_updates])
      end
    end

    test 'pod watch ignore MODIFIED on id cache if info is not cached' do
      @client.stub :watch_pods, [@modified] do
       start_pod_watch
       assert_equal(true, @cache.key?('modified_uid'))
       assert_equal(1, @stats[:pod_cache_watch_updates])
    ...
    test 'pod watch updates id cache when MODIFIED is received and pod uid is changed' do
      cache_key = get_id_cache_key_given_metadata_source
      @id_cache[cache_key] = { :pod_id => 'original_uid' }
      @client.stub :watch_pods, [@modified] do
       start_pod_watch
       assert_equal('modified_uid', @id_cache[cache_key][:pod_id])
       assert_equal(1, @stats[:id_cache_watch_updates_pod])
      end
    end
